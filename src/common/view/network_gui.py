...
def receive():
    ...
    if width != 0 and height != 0:
        try:
            ...
            full_proj_transform = torch.reshape(torch.tensor(message["view_projection_matrix"]), (4, 4))
            full_proj_transform[:,1] = -full_proj_transform[:,1]
            
            _vrame =gsframe.ViewFrame
            _vrame.fovY = fovy
            _vrame.fovX = fovx
            _vrame.width = width
            _vrame.height = height

            _vrame.w2cTensor = world_view_transform.T
            _vrame.w2cViewTensor = full_proj_transform.T
            _c2w = torch.inverse(_vrame.w2cViewTensor)
            _vrame.c2wCenterTensor = _c2w[:3, 3]
            ...
            # raise e
        return {'frame': _vrame,
                'train': bool(message["train"]),
                'alive': bool(message["keep_alive"]),
